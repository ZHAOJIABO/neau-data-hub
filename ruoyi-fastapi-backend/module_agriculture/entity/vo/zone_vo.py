...
class IrrigationZoneModel(BaseModel):
    ...
    iwue: float | None = Field(default=0.0, ge=0, description='年度灌溉水利用系数 IWUE')
    water_productivity_kg_m3: float | None = Field(default=0.0, ge=0, description='年度水分生产率 WUE (kg/m³)')
    benefit_yuan_per_m3: float | None = Field(default=0.0, ge=0, description='年度单方水净效益 BEC (元/m³)')
    irrigation_reliability: float | None = Field(default=0.0, ge=0, le=1, description='年度灌溉保证率 IRS')
    field_efficiency: float | None = Field(default=0.0, ge=0, le=1, description='年度田间水利用系数 FE')
    surface_water_utilization: float | None = Field(default=0.0, ge=0, le=2, description='年度地表水利用率 SUR')
    groundwater_utilization: float | None = Field(default=0.0, ge=0, le=2, description='年度地下水利用率 GWR')
    groundwater_dependency: float | None = Field(default=0.0, ge=0, le=1, description='年度地下水依赖度 GWI')
